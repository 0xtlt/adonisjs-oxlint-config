...
import {
  ...
  writeOxlintConfig,
  writeProjectFile,
} from '../helpers/run_oxlint'

function ruleNames(stdout: string, stderr: string): string[] {
  const fromJson = parseOxlintJson(stdout)
    .diagnostics.map((item) => item.code ?? item.rule ?? '')
    .filter(Boolean)
  ...
  const matches = combined.matchAll(
    /\b((?:eslint|typescript|unicorn|node|react|vue|@adonisjs)\/[\w-]+|[\w-]+)\s*\(/g
  )
