...
function reportIfUndefinedInit(
  context: { report: (descriptor: { node: unknown; messageId: string; data: { name: string }; fix: (fixer: { replaceText: (node: unknown, text: string) => unknown }) => unknown }) => void },
  declarator: VariableDeclaratorNode
): void {
  if (!isIdentifier(declarator.id) || !declarator.init || !isIdentifier(declarator.init, 'undefined')) {
    return
  }
