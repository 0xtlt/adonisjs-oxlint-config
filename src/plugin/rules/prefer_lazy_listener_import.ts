import { type ArrayExpressionNode, type CallExpressionNode, type ImportDeclarationNode, isIdentifier, isMemberCall } from '../ast'
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node: ImportDeclarationNode) {
        ...
        if (node.source.value === '@adonisjs/core/services/emitter' && node.specifiers[0]?.type === 'ImportDefaultSpecifier') {
        ...
        for (const element of secondArgument.elements) {
          ...
          context.report({
            ...
            fix(fixer) {
              ...
              return fixer.replaceText(importNode, `const ${element.name} = () => import(${importPath})`)
            },
          })
        }
      },
