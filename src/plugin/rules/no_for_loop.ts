...
function isIndexLengthLoop(node: ForStatementNode): boolean {
  const init = node.init as { type?: string; declarations?: Array<{ id?: { name?: string } }> } | null
  ...
  const update = node.update as { type?: string; operator?: string; argument?: { name?: string }; prefix?: boolean } | null
  ...
  const comparesLength = test.right?.type === 'MemberExpression' && test.right.property?.name === 'length'
  ...
  if (update.type === 'UpdateExpression') {
    return update.argument?.name === indexName && (update.operator === '++' || update.operator === '--')
  }
