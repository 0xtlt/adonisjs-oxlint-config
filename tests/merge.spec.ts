...
describe('mergeConfig', () => {
  it('lets later rules override earlier rules', () => {
    const merged = mergeConfig({ rules: { eqeqeq: 'error', curly: 'error' } }, { rules: { curly: 'off' } })
  ...
  it('concatenates ignore patterns without duplicates', () => {
    const merged = mergeConfig({ ignorePatterns: ['dist/**', 'build/**'] }, { ignorePatterns: ['build/**', 'coverage/**'] })
  ...
  it('merges settings objects', () => {
    const merged = mergeConfig({ settings: { react: { version: '18.0' } } }, { settings: { react: { version: '19.0' } } })
