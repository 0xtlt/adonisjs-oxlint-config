...
export function writeProjectFile(directory: string, relativePath: string, contents: string): string {
...
export function runOxlint(directory: string, args: string[] = []): { stdout: string; stderr: string; status: number } {
  try {
    const stdout = execFileSync(
      process.execPath,
      [join(process.cwd(), 'node_modules/oxlint/bin/oxlint'), '--format', 'unix', ...args],
      {
        cwd: directory,
        encoding: 'utf8',
